...
test.describe('Admin Page', () => {
  ...
  const empFirstName: string = 'dummy';
  const empMiddleName: string = 'bot';
  const empLastName: string = empId.toString();
  const empPassword: string = 'ABCabc123!';
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    await appUI.pimPage.addEmployeeForm.addNewEmployee(
      { firstName: empFirstName, middleName: empMiddleName, lastName: empLastName, employeeId: empId }
    )
    await page.close();
  })
  ...
  test('Should be able to add a new user', async ({ authAppUI }) => {
    const empUsername: string = `dummy-employee-${empId}`;
